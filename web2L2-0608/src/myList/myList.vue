<template>
    <div id="myList" style="width: 1000px;margin: 0 auto">
        <div style="margin-left: 40%; width: 900px;margin-top: 20px;"></div>
        <card>
            <List item-layout="vertical">
                <!-- <h1 v-for="item in res" :key="item.userName">{{item.sex+item.age}}-</h1> -->
                <ListItem v-for="item in res" :key="item.userName" style="height:150px">
                    <ListItemMeta :avatar="item.avatar" :title="item.userName" :description="item.date" />
                    ...
                    婚姻：<font color="blue" style="text-decoration:underline">{{item.marriage}}</font> &nbsp;
                    民族：<font color="blue" style="text-decoration:underline">{{item.nation}}</font>&nbsp; 
                    过敏史：<font color="blue" style="text-decoration:underline">{{item.allergy}}</font>&nbsp; 
                    住址：<font color="blue" style="text-decoration:underline">{{item.address}}</font>  &nbsp;
                    主诉：<font color="blue" style="text-decoration:underline">{{item.textarea}}</font><br><br>
                    ...
                    <template slot="extra">
                        <img v-image-preview :src="item.imgSrc" style="width: 230px;height:130px">
                    </template>
                </ListItem>
            </List>
        </card>
...
<script>
    export default {
        ...
        methods:{
            ...
            getIfo(){
                let that = this;
                this.axios.get('http://139.159.209.32:8909/analysisController/getAllMRByDoctor').then(res=>{
                    res  = res.data;
                    that.allInfo=res;
